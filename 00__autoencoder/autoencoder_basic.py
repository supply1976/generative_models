...
def main():
  ...
  parser.add_argument('--enc_inter_dims', type=int, nargs="*", default=[128])
  parser.add_argument('--dec_inter_dims', type=int, nargs="*", default=[128])
  ...
  autoencoder = AutoEncoder(FLAGS.latent_dim, inp_shape)
  # build and initialize encoder model
  encoder_model = autoencoder._encoding(_units=FLAGS.enc_inter_dims)
  encoder_model.summary()
  for ly in encoder_model.layers:
    print(ly.name, ly, ly.output.shape)

  # build initialize decoder model
  decoder_model = autoencoder._decoding(_units=FLAGS.dec_inter_dims)
  ...
  x_encoded = encoder_model(autoencoder.x)
  x_decoded = decoder_model(x_encoded)
  AE_model = keras.Model(inputs=autoencoder.x, outputs=x_decoded, name="AE")
  ...
  AE_model.add_metric(latent_vec_norm_loss, name='latent_vec_norm_loss', aggregation='mean')
  ...
  x_dec_b4tr = AE_model.predict(test_images[0:10])
  ...
  if FLAGS.training:
    AE_model.fit(x=train_images, y=train_images,
      epochs=FLAGS.epochs,
      shuffle=True,
    ...
    x_dec_aftr = AE_model.predict(test_images[0:10])
    data = np.concatenate([test_images[0:10], x_dec_b4tr, x_dec_aftr])
    ...
    ae_model_path = os.path.join(savedir, "best_ae_model.h5")
    ...
    test_decoded = restored_AE_model.predict(test_images[rnd_idx])
    data = np.concatenate([test_images[rnd_idx], test_decoded])
    view_images(data, ncols=10)
    # see the encoded latent vector distribution
    enc_vec = restored_encoder_model.predict(test_images)
    view_encoded_result(enc_vec, test_labels)
