...
def resize_and_rescale(img, img_size, rescale=True):
...
class AttentionBlock(keras.layers.Layer):
    ...
    def __init__(self, units, groups=8, **kwargs):
        """
        # GroupNormalization() is only available after TF 2.12
        # LayerNormalization() has bug in p5ocnda3 TF 2.5 
        """
        self.units = units
        self.groups = groups
...
def main():
    ...
    parser.add_argument('--tfdataset', type=str, default=None, 
      help='tf dataset name, available: "oxford_flowers102, cifar10, mnist')
    parser.add_argument('--dbfile', type=str, default=None) 
    parser.add_argument('--mydbname', type=str, default="custom") 
    ...
    if FLAGS.tfdataset is not None:
        if FLAGS.tfdataset == 'cifar10':
            (train_inputs, _), (test_inputs, _) = keras.datasets.cifar10.load_data()
        elif FLAGS.tfdataset == 'mnist':
            (train_inputs, _), (test_inputs, _) = keras.datasets.mnist.load_data()
        elif FLAGS.tfdataset == 'fashion_mnist':
            (train_inputs, _), (test_inputs, _) = keras.datasets.fashion_mnist.load_data()
        else:
            print("no dataset '{}' in keras.datasets".format(FLAGS.tfdataset))
            return 
        # unit8 (0-255) to float32 (0~1) 
        images = train_inputs.astype(np.float32)
        images = images / 255.0
        dbname = FLAGS.tfdataset
    else:
        if FLAGS.dbfile is None:
            images = np.load("./random_patterns.npy")
            print("use fake random patterns {} for flow testing".format(images.shape))
        elif dbfile.endswith(".npz"):
            data = np.load(dbfile)
            images = data['images']
            print("{} has shape {}".format(dbfile, images.shape))
            dbname = FLAGS.mydbname
        else:
            print("not support")
            return
    ...
    if FLAGS.restore_model and FLAGS.model_dir is not None:
        model_path = os.path.join(FLAGS.model_dir, "ema_best")
