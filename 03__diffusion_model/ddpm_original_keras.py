...
def build_model(
    ...
    image_input = keras.Input(shape=(img_size, img_size, img_channels), name="image_input")
    ...
    x = activation_fn(x)
    x = keras.layers.Conv2D(img_channels, (3, 3), padding="same", kernel_initializer=kernel_init(0.0))(x)
...
def main():
    ...
    norm_groups = FLAGS.norm_groups  # Number of groups used in GroupNormalization layer
    ...
    if FLAGS.restore_model and FLAGS.model_dir is not None:
        fd = open(os.path.join(FLAGS.model_dir, 'checkpoint'), 'r')
        model_path = fd.readline()
        model_path = model_path.replace('"', '').strip().split(": ")[1]
        model_path = os.path.join(FLAGS.model_dir, model_path)
