...
def main():
  ...
  cols = ["KLD_gimg", "KLD_psd", "FID_inceptV3", 
    "net", 'scheduler', "loss", "pred", "steps", "clipDN", "gen_date", "batch", "repeat", "LR"]
  ...
  for i, csv in enumerate(gen_csvs):
    ...
    nrb = net_dict['NUM_RESNET_BLOCKS']
    ...
    if "_raw_" in csv:
      ...
      clipDN = 1
    tr_cond = [nn_code, sch, loss_fn, pred_type, rev_steps, clipDN, gen_date, batch, dsrept, lr]
    # get features
