...
class DiffusionModel(keras.Model):
    ...
    def call(self, inputs):
        ...
        noise = tf.random.normal(shape=tf.shape(inputs), dtype=tf.float32)
        xt_samples = self.gdf_util.q_sample(inputs, t, noise)
        pred_noise = self.network([xt_samples, t], training=True)
        loss = tf.reduce_mean(keras.losses.mse(noise, pred_noise))
        self.add_loss(loss)
    ...
    """ TF v2.1 has no train_step method, 
...
#axes = axes.flatten()
#for i in range(4):
#  axes[i].imshow(train_images.numpy()[i])
#plt.show()
...
model.ema_network.trainable = False
...
model.ema_network.save_weights("./best/ckpt")
